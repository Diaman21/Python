...
n_num = float(input('Введите вещественное число: '))
summ = 0
if(n_num<0):
    n_num= -n_num    
...
for i in range(0,len(people_list)-1):
    ...
    if(counter == len(people_list)):
        last_player_def = 0
    print(f'Игрок: {people_list[counter-1]} выбывает.\n{money_list[counter-1]} монетки достаются {people_list[last_player_def]} игроку')    
    if(counter == len(people_list)):
    ...
    if(len(people_list) > 1):
        ...
        if(quit=='N' or quit=='n'):
            print('Ты ушел так и не узнав победителя')
            break
    else: print(f'Победил игрок {people_list}, унеся с собой {money_list} монет')
